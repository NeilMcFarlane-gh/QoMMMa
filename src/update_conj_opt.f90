...
real(dp) ::  maxchgx, mod_dire, mod_delx
	 ...
	 if_climbing=.false.
        ...
        if (.not. if_climbing .and. (hh.lt.0.)) then ! Not good, check not necessary in CI-NEB
            ...
            ! Check for small changes for each image?
             do img_num=2,nimg           
                 if ((maxval(abs(ChgeX(img_num,:))).lt.(0.25*toldxmax_org)) &
                     .and.(norm_per_force(img_num).lt.tolper).and.(.not.climbing(img_num))) then           
                     ChgeX(img_num,:) = 0.0d0           
        ...
        !else if (ratio.gt. 0.33) then
                ...
                !chgex = 0.7 * dire
            !weight = hh
            ChgeX = 1.5/weight * dire ! for the first and last image this is equal to zero 
            weight = weight/1.5
            !call check_step_length(chgex(:,:),weight)
        ...
        !else if (ratio.lt.-0.33) then
            ...
            ! Added 10/10/05 JZ
            if (gam .lt. 0.) gam=0.
            ! Don't do this check for climbing image
            ! added 31/01/06
            if ((check_restart .gt. 0.5) .or. (check_restart .lt. -0.5).and. (.not. if_climbing)) gam=0.
            !if ((check_restart .gt. 0.5) .or. (check_restart .lt. -0.5)) gam=0.
            ...
            do img_num=2,nimg
                if ((maxval(abs(ChgeX(img_num,:))).lt.(0.25*toldxmax_org)) &
                    .and.(.not.climbing(img_num)).and.(norm_per_force(img_num).lt.tolper)) then
                    ChgeX(img_num,:) = 0.0d0
                    update_geom(img_num)=.false.
                    h = oh
                end if
...
do img_num=1,nimg
    ...
    if (norm_per_force(img_num).lt.tolper) then
             convs(6)="YES" ; i=i+1
    end if
             
    if (i .eq. 6) converged = .true.
...
real(dp) :: new_weight
real(dp) :: stpl, lstep, stpmax, stpmx
