...
do img_num=1,nimg
	...
	if (disp.eq.1) then
	  call disp_corr()
	  fullqend(img_num) = qe
      write (*,'(A,F12.6,A)') "QM energy without dispersion= ", qe, " Hartree"
	  qe = qe + edistot
	  write (*,'(A,F12.6,A)') "QM energy with dispersion= ", qe, " Hartree"
	  qg = qg + dispgrad
	end if
	...
	do i = 1, nl
		...
		qg(j:j+2) = lratio(i) * (vec)/bohr
		qg(k:k+2) = qg(k:k+2) + (1.d0-lratio(i)) * (vec)/bohr
	end do
