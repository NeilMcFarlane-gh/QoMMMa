SUBROUTINE read_checkfile_mecp()
...
if (nstep .eq. 0) then
     ...
     read (unit=8,fmt=*) dummy
     read (unit=8,fmt='(F15.8)') oea
     read (unit=8,fmt=*) dummy
     read (unit=8,fmt='(F15.8)') oeb
     read (unit=8,fmt=*) dummy
...
! Copy data from single-image arrayes into all-image array
...
END SUBROUTINE read_checkfile_mecp
