...
do img_num=1,nimg
	...
	open(unit=9,file=("nqmgeom"//trim(img_string(img_num))//".xyz"),status="replace")

	do i=1,nq
		 j = 3*(i-1)+1
		 write(unit=9,fmt='(A2,2X,3F12.6)') qlabel(i),xq(j),xq(j+1),xq(j+2)
	end do
	do i=1,nl
		 j=3*(i-1)+1
		 write(unit=9,fmt='(A2,2X,3F12.6)') llabel(i),xl(j),xl(j+1),xl(j+2)
	end do
