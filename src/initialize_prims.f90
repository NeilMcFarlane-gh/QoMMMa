...
if (coordtype .eq. 1) then
	...
	if (.not. ALLOCATED(to_generate)) allocate(to_generate(nopt))
	...
	if (primtype .eq. 0) then ! Total connectivity
		call define_prims_TC(nopt, to_generate, prim_list)
	else if (primtype .eq. 1) then ! Full definition
		call define_prims_full(nopt, to_generate, xopt, prim_list)	
	end if
