...
real(sp) :: dist, angle, arccos, tot_angle
logical :: any
conv_counter=0
...
write (unit=8,fmt='(A,1X,2F13.6)') "Present, previous energy of state A:",ea,oea
write (unit=8,fmt='(A28,1X,2F13.6)') "QM, MM, energies of state A:",qea,te
write (unit=8,fmt='(A,1X,2F13.6)') "Present, previous energy of state B:",eb,oeb
write (unit=8,fmt='(A28,1X,2F13.6)') "QM, MM, energies of state B:",qeb,te
if (ncon.gt.0) then
...
if (climbing(img_num)) then
    write (unit=8,fmt=100) "Difference in energy:",conv(1),tolde_org,convs(1)
    ...
    write (unit=8,fmt=100) "Difference in energy:",conv(1),tolde,convs(1)
...
if ((nebtype.eq.1).or.(nebtype.eq.2).or.(nebtype.eq.3).or.(nebtype.eq.5)) then
    ...
    write(unit=9,fmt='(A20,A14,A2,A14,A11,A7,1X,A13,5A7,A14)') "","previous image", " ", "between images",&
         "in degrees", "angle", "energy  A", "change", "dX ", "dX ", "grad ", "grad ", "component"
    do img_num=1,nimg
        if (img_num.eq.1) then
            dist=0
            angle=0
...
! lines were splited in two to complie in g95
        ...
        write (unit=9,fmt='(A,I4,A,F14.6, F14.3, F12.3, F9.3, F13.6,5A7, F13.6)') "Image number ", img_num, ": ",dist, angle, &
             arccos*180/pi, tot_angle, fullea(img_num),convs(1), convs(2), & 
...
END SUBROUTINE write_report_mecp
