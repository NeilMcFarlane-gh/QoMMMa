...
! In this subroutine, the step(s) in primitive internal coordinates are taken.
! If a primitive coordinate is provided which is not automatically generated by initialize_prims, then it is added to prim_list.


integer(i4b) :: i, j, k, l, m, ii, in_pos, old_nprim
integer(i4b) :: qm_temp, prim_list_work(nprim,4), to_add
...
if (disp_prim .gt. 0) then
	...
	to_add = 0
	do i=1, disp_prim
		prim_temp1 = dq_at_p(i,:)
		do j=1, nprim
			prim_temp2 = prim_list_work(j,:)
			if (MAXVAL(ABS(prim_temp1 - prim_temp2)) .eq. 0) then ! the primtive internal coordinate has already been generated so no need to add it.
				exit
	...
	do i=1, disp_prim
		...
		do j=1, old_nprim
			prim_temp2 = prim_list_work(j,:)
			if (MAXVAL(ABS(prim_temp1 - prim_temp2)) .eq. 0) then
				! At this point, the position which the coordinate is at is recorded so that the step can be recorded.
				in_pos = j
				dq_pos_p(i) = in_pos
				exit
