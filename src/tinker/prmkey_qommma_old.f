...
c     
c     my hack - only read in the difference gradient if use_extra is set
c     
         if (use_extra) then
            call readdiffgrad(use_extra)
         end if
...
c
      ...
      else if (keyword(1:11) .eq. 'MM2-STRBND ') then
         mm2stbn = .true.
c
...
c
         ...
         read (string,*,err=10,end=10)  pgamma
