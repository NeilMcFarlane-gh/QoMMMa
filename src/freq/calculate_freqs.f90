...
DOUBLE PRECISION, ALLOCATABLE :: Q(:),aw(:),h(:,:),mwhess(:,:),eigvx(:,:),eigs(:)
DOUBLE PRECISION, ALLOCATABLE :: freqau(:),freqwn(:),eigv(:,:)
...
n=3*ns
allocate(q(n),h(n,n),aw(n),at(ns),mwhess(n,n),eigvx(n,n),eigs(n))
allocate(freqau(n),freqwn(n),imag(n),lab(ns),eigv(n,n))

CALL ReadInput(ns,n,q,h,aw,lab)
